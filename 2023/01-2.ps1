...
function Get-Replaced($Dict, $Text) {
    $Dict.GetEnumerator()|%{
...
function Invoke-StringNumeriser($Dict, $Text) {
    # I can't believe you've done this
    $r = [Regex]::new('two|eight|one|three|nine')
    $mc = $r.Matches($Text)
    $mc|%{
        $match = $_.Value
        $Text = $Text.Replace($match,$Dict[$match])
    }
    $Text
}

function Invoke-BigDirtyHack($HackyDict, $Text) {
    Get-Replaced -Dict $HackyDict -Text $Text
}
...
$input.Split("`n") | ForEach-Object {
    $numerised =  Invoke-BigDirtyHack -Text $_ -HackyDict $hackydict
    $fixed = Get-Replaced -Dict $dict -Text $numerised
    $arr = ($fixed -replace '[a-z]','').ToCharArray()
    $first = ($arr | Select-Object -First 1)
    $last = ($arr | Select-Object -Last 1)
    [PSCustomObject]@{
        Input = $_
        Numerised = $numerised
        NumbersFixed = $fixed
...
} | Measure-Object -Property Concat -Sum
