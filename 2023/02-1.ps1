...
function Get-LineParsed($line) {
    $LineSplit = $_.Split(":")
    $GameID = $LineSplit[0].Split(" ")[1]
    $Games = [System.Collections.ArrayList]::new()
    $GamesAsText = $LineSplit[1].Split(";")|%{$_.Trim()}
    $GamesAsText | % {
        $GameResults = @{
            "red"=0
            "blue"=0
        ...
        $mc = $r.Matches($_)
        forEach($m in $mc) {
            $GameResults[$m.Groups[2].Value] = $m.Groups[1].Value
        }
        [void]$Games.Add($GameResults)
    }
    [PSCustomObject]@{
        ID = $GameID
        Games = $Games
        RedMax = ($Games.red | Measure-Object -Maximum).Maximum
        BlueMax = ($Games.blue | Measure-Object -Maximum).Maximum
        GreenMax = ($Games.green | Measure-Object -Maximum).Maximum
    }
}

$textinput.Split("`n") | % {
    $parsed = Get-LineParsed($_)
    $possible = $true
    if ($parsed.RedMax -gt $cubes.red) {$possible = $false}
    if ($parsed.BlueMax -gt $cubes.blue) {$possible = $false}
    if ($parsed.GreenMax -gt $cubes.green) {$possible = $false}
    [PSCustomObject]@{
        ID = $parsed.ID
        Possible = $possible
    }
} | Where Possible -eq $true | Measure-Object -Property ID -Sum