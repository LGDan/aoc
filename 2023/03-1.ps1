...
function Get-CharAt($parsed, $x, $y) {
    ([Char[]]$parsed[$y])[$x]
}

function Get-NumberAtByCrawling($parsed, [Int]$x, [Int]$y, [String]$OriginXY) {
    $s = Get-CharAt -parsed $parsed -x $x -y $y
    forEach($n in -1..-3) { #Backward First
        $c = Get-CharAt -parsed $parsed -x ($x+$n) -y $y
        if ($c -match '\d') {$s = $c.ToString() + $s.ToString()}else{
            break
        }
    }

    forEach($n in 1..3) { #Then Forward
        $c = Get-CharAt -parsed $parsed -x ($x+$n) -y $y
        if ($c -match '\d') {$s = $s.ToString() + $c.ToString()}else{
            break
        }
    }
...
function Get-IsValidNumber($parsed, [Int]$x, [Int]$y, [Int]$numberLength, $lineCount, $lineLength) {
    ...
    $valid = $false
    $i = 0
    $points | Where {($_.X -lt $lineLength) -and ($_.Y -lt $lineCount) -and ($_.Y -ge 0) -and ($_.X -ge 0)} | % {
        $charAt = (Get-CharAt -parsed $parsed -x $_.X -y $_.Y)
        #Write-Host -NoNewline "$charAt"
        if ($charAt -notmatch '\d|\.') {
            $valid = $true
        }
        $i++
    }
...
function Invoke-V2($schematic) {
    $parsed1 = $schematic.Split("`n")
    $lineCount = $parsed1.Length
    $lineLength = $parsed1[0].Length

    for($y=0;$y-lt$lineCount;$y++) {
        for($x=0;$x-lt$lineLength;$x++) {
            $char = ([Char[]]$parsed1[$y])[$x]
            if ($char -match "\d") {
                $crawlResult = Get-NumberAtByCrawling -x $x -y $y -parsed $parsed1
                $number = $crawlResult.Number
                [PSCustomObject]@{
                    Number = $number
                    X = $x
                    Y = $y
                    Valid = (Get-IsValidNumber -parsed $parsed1 -x $x -y $y -numberLength $number.ToString().Length -lineCount $lineCount -lineLength $lineLength)
                }
...
$res = Invoke-V2 -schematic $textinput

[PSCustomObject]@{
    Count = ($res | Measure-Object).Count
    ValidCount = ($res | Where Valid -eq $true | Measure-Object).Count
    Sum  = ($res | Where Valid -eq $true | Measure-Object -Property Number -Sum).Sum
    Valid = ($res | Where Valid -eq $true| Select -ExpandProperty Number)
    Invalid = ($res | Where Valid -eq $false| Select -ExpandProperty Number)
}
