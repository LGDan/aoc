...
function Get-CandidateGears($schematic) {
    $parsed1 = $schematic.Split("`n")
    $lineCount = $parsed1.Length
    $lineLength = $parsed1[0].Length
    $gears = [System.Collections.ArrayList]::new()
    for([Int]$y=0;$y-lt$lineCount;$y++) {
        for([Int]$x=0;$x-lt$lineLength;$x++) {
            $char = ([Char[]]$parsed1[$y])[$x]
            if ($char -match "\*") {
                [void]$gears.Add([PSCustomObject]@{
                    X = $x
                    Y = $y
                })
            }
...
function Get-GearRatios($schematic, $Candidate, $lineCount, $lineLength) {
    $parsed1 = $schematic.Split("`n")
    $lineCount = $parsed1.Length
    $lineLength = $parsed1[0].Length
    ...
    $discoveredNumbers = [System.Collections.ArrayList]::new()
    $points | Where-Object {
        ($_.X -lt $lineLength)
         -and ($_.Y -lt $lineCount)
         -and ($_.Y -ge 0)
         -and ($_.X -ge 0)} | ForEach-Object {
        $charAt = (Get-CharAt -parsed $parsed1 -x $_.X -y $_.Y)
        if ($charAt -match '\d') {
            [pscustomobject]@{
                X = $_.X
                Y = $_.Y
        ...
        [void]$discoveredNumbers.Add(
            (Get-NumberAtByCrawling -parsed $parsed1 -x $_.X -y $_.Y -OriginXY "$GearX;$GearY")
        )
    }

    if (($discoveredNumbers.Number | Select-Object -Unique|Measure-Object).Count -eq 2) {
        $numbers = $discoveredNumbers.Number | Select-Object -Unique
        [PSCustomObject]@{
            GearX = $GearX
            GearY = $GearY
            Numbers = $numbers
...
(Get-CandidateGears -schematic $textinput).Gears | ForEach-Object {
    Get-GearRatios -schematic $textinput -Candidate $_
} | Measure-Object -Property Ratio -Sum
