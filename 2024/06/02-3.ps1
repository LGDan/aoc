...
function Process-Grid($inputgrid) {
    # Locate the Guard
    $state = ""
    ...
    | ForEach-Object {
        ...
        do {
            ...
            $i++
        } until ($GuardState.End -or ($i -gt 10000))

        $state = $GuardState.End
    }
    [PSCustomObject]@{
        State = $state
    }
...
| ForEach-Object {
    ...
    $tryCoords = $_.Split(",")|%{[int]$_}
    $chrArray = $newGrid[$tryCoords[1]].ToCharArray()
    $chrArray[$tryCoords[0]] = "#"
    $newGrid[$tryCoords[1]] = $chrArray -join ""
    [PSCustomObject]@{
        XY = $tryCoords
        Result = Process-Grid -inputgrid $newGrid
    }
}
| Where-Object {$_.Result.State -eq $false}
| Measure-Object